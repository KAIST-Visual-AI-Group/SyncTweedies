...
__global__ void isect_tiles(
    ...
    uint2 tile_pos; // = make_uint2((uint32_t)floor(tile_x), (uint32_t)floor(tile_y));
    tile_pos.x = min(max(0, (uint32_t)floor(tile_x)), tile_width - 1);
    tile_pos.y = min(max(0, (uint32_t)floor(tile_y)), tile_height - 1);
    ...
    int32_t i = tile_pos.y;
    int32_t j = tile_pos.x;
    ...
    if (total_elems) {
        AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, means2d.scalar_type(), "isect_tiles_total_elems", [&]() {
            isect_tiles<<<(total_elems + N_THREADS - 1) / N_THREADS, N_THREADS, 0,
                ...
                radii.data_ptr<int32_t>(), depths.data_ptr<scalar_t>(), nullptr,
    ...
    if (n_isects) {
        AT_DISPATCH_FLOATING_TYPES_AND2(at::ScalarType::Half, at::ScalarType::BFloat16, means2d.scalar_type(), "isect_tiles_n_isects", [&]() {
            isect_tiles<<<(total_elems + N_THREADS - 1) / N_THREADS, N_THREADS, 0,
                ...
                cum_tiles_per_gauss.data_ptr<int64_t>(), tile_width, tile_height,
