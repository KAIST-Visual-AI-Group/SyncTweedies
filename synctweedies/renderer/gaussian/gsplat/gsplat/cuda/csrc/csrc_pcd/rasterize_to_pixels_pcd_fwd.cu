...
__global__ void rasterize_to_pixels_fwd_kernel(
    ...
    if (range_end < range_start) {
        // print cid, tid, i, j, range_start, range_end
        printf("cid, tid, i, j, range_start, range_end: %d, %d, %d, %d, %d, %d\n", camera_id, tile_id, i, j, range_start, range_end);
    }
...
#define __GS__CALL_(N)                                                                 \
    case N:                                                                            \
        return call_kernel_with_dim<N>(means2d, colors, opacities,             \
                                       backgrounds, image_width, image_height,         \
                                       tile_offsets, flatten_ids, sqrt_mode);
