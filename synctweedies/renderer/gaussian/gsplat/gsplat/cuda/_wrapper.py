...
def fully_fused_projection_pcd(
    means: Tensor,  # [N, 3]
    viewmats: Tensor,  # [C, 4, 4]
    Ks: Tensor,  # [C, 3, 3]
    width: int,
    ...
    This function fuse the process of transforming to camera space (:func:`world_to_cam()`),
    and perspective projection (:func:`persp_proj()`).
    ...
    if packed:
        (
            ...
            means2d,
            depths,
        ) = _make_lazy_cuda_func("fully_fused_projection_packed_pcd_fwd")(
            means,
            viewmats,
            Ks,
            width,
        ...
        return _make_lazy_cuda_func(
            "fully_fused_projection_pcd_fwd"
        )(
            means,
            viewmats,
            Ks,
...
def isect_tiles_pcd(
    ...
    .. note::
        ...
        Although radii does not exist for point cloud data, it is still required as an input
        to indicate the 'valid' points (i.e. the point is inside the frustum).
    ...
    tiles_per_gauss, isect_ids, flatten_ids = _make_lazy_cuda_func("isect_tiles_pcd")(
        means2d.contiguous(),
        radii.contiguous(),
        depths.contiguous(),
        camera_ids,
...
def rasterize_to_pixels_pcd(
    ...
    Args:
        ...
        backgrounds: Background colors. [C, channels]. Default: None.
        packed: If True, the input tensors are expected to be packed with shape [nnz, ...]. Default: False.
        sqrt_mode: If True, the blending weights will be square-rooted. Default: False.
