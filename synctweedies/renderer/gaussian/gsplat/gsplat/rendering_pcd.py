...
from .cuda._torch_impl_pcd import (
    _fully_fused_projection_pcd,
    _isect_offset_encode,
    _isect_tiles_pcd,
)

from .cuda._wrapper import (
    rasterize_to_pixels_pcd,
    isect_tiles_pcd
)
...
def rasterization(
    ...
    proj_results = _fully_fused_projection_pcd(
        means,
        viewmats,
        Ks,
        width,
    ...
    if packed:
        ...
        radii, means2d, depths = proj_results
    ...
    tile_width = math.ceil(width)
    tile_height = math.ceil(height)
    ...
    tiles_per_gauss, isect_ids, flatten_ids = isect_tiles_pcd(
        means2d,
        radii,
        depths,
        tile_width,
    ...
    isect_offsets = isect_offset_encode(isect_ids, C, tile_width, tile_height)
    # return torch.zeros(1, 1, 1, 1), torch.zeros(1, 1, 1, 1), {'isect_offsets': isect_offsets, 'isect_ids': isect_ids}
    ...
    if sh_degree is None:
        ...
        if packed:
            ...
            masks = torch.ones(gaussian_ids.shape[0], dtype=torch.bool, device=means.device)  # [nnz]
            if colors.dim() == 3:
            ...
            masks = torch.ones((C, N), dtype=torch.bool, device=means.device)
            if colors.dim() == 3:
    ...
    if colors.shape[-1] > channel_chunk:
        ...
        for i in range(n_chunks):
            ...
            render_colors_, render_alphas_ = rasterize_to_pixels_pcd(
                means2d,
                colors_chunk,
                opacities,
                width,
        ...
        render_colors, render_alphas = rasterize_to_pixels_pcd(
            means2d,
            colors,
            opacities,
            width,
