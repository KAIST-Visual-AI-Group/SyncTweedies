...
def get_extensions():
    ...
    extensions_dir_v3 = osp.join("gsplat", "cuda", "csrc", "csrc_pcd")
    sources_v3 = glob.glob(osp.join(extensions_dir_v3, "*.cu")) + glob.glob(
        osp.join(extensions_dir_v3, "*.cpp")
    )
    sources_v3 = [path for path in sources_v3 if "hip" not in path]

    # merge sources_v3 with sources_v2 to construct a single extension
    extensions_dir_v2 = extensions_dir_v2 + extensions_dir_v3
    sources_v2 = sources_v2 + sources_v3
